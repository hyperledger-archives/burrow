...
func (st *Stack) useGas(gasToUse uint64) {
	if *st.gas > gasToUse {
		*st.gas -= gasToUse
		...
		st.pushErr(errors.Code.InsufficientGas)
	}
}
...
func (st *Stack) Push(d Word256) {
	...
	if err != nil {
		st.pushErr(errors.Code.DataStackOverflow)
		return
	}
	st.slice[st.ptr] = d
	st.ptr++
...
func (st *Stack) Pop() Word256 {
	st.useGas(native.GasStackOp)
	if st.ptr == 0 {
		st.pushErr(errors.Code.DataStackUnderflow)
		return Zero256
	}
	st.ptr--
...
func (st *Stack) Pop64() int64 {
	...
	if Is64BitOverflow(d) {
		st.pushErr(errors.Errorf(errors.Code.IntegerOverflow, "int64 overflow from word: %v", d))
		return 0
	}
...
func (st *Stack) PopU64() uint64 {
	d := st.Pop()
	if Is64BitOverflow(d) {
		st.pushErr(errors.Errorf(errors.Code.IntegerOverflow, "uint64 overflow from word: %v", d))
		return 0
	}
