...
func TestEVM(t *testing.T) {
	...
	t.Run("SendCall", func(t *testing.T) {
		...
		require.Equal(t, errors.Code.InsufficientBalance, errors.GetCode(exCalls[0].Header.Exception))
	...
	t.Run("StaticCallReadOnly", func(t *testing.T) {
		...
		for _, illegalContractCode := range []acm.Bytecode{
			...
			require.Equal(t, errors.Code.IllegalWrite, txe.Exception.ErrorCode(),
	...
	t.Run("Invalid", func(t *testing.T) {
		...
		output, err := call(vm, st, account1, account2, bytecode, nil, &gas)
		assert.Equal(t, errors.Code.ExecutionAborted, errors.GetCode(err))
		t.Logf("Output: %v Error: %v\n", output, err)
	})
	...
	t.Run("CallNonExistent", func(t *testing.T) {
		...
		require.Equal(t, errors.Code.NonExistentAccount, errors.GetCode(ex),
	...
	t.Run("GetBlockHash", func(t *testing.T) {
		...
		require.Equal(t, errors.Code.InvalidBlockNumber, errors.GetCode(err),
		...
		_, err = vm.Execute(st, blockchain, eventSink, params, bytecode)
		require.Equal(t, errors.Code.BlockNumberOutOfRange, errors.GetCode(err),
	...
	t.Run("DataStackOverflow", func(t *testing.T) {
		...
		*/
		...
		require.Equal(t, errors.Code.DataStackOverflow, errors.GetCode(ex), "Should be stack overflow")
	})
	...
	t.Run("CallStackOverflow", func(t *testing.T) {
		...
		*/
		...
		require.Equal(t, errors.Code.ExecutionReverted, errors.GetCode(callError))
		// Errors are post-order so first is deepest
		require.True(t, len(callError.NestedErrors) > 0)
		deepestErr := callError.NestedErrors[0]
		require.Equal(t, errors.Code.CallStackOverflow, errors.GetCode(deepestErr))
...
func (b *blockchain) BlockHash(height uint64) ([]byte, error) {
	if height > b.blockHeight {
		return nil, errors.Code.InvalidBlockNumber
	}
	bs := make([]byte, 32)
