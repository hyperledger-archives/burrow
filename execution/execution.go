...
func (exe *executor) validateInputsAndStorePublicKeys(txEnv *txs.Envelope) error {
	for s, in := range txEnv.Tx.GetInputs() {
		...
		if acc.Sequence+1 != uint64(in.Sequence) {
			return errors.Errorf(errors.Code.InvalidSequence, "Error invalid sequence in input %v: input has sequence %d, but account has sequence %d, "+
		...
		if !v {
			return errors.Code.NoInputPermission
		}
	}
	return nil
}
