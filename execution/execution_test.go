...
func newBlockchain(genesisDoc *genesis.GenesisDoc, state *State) bcm.MutableBlockchain {
	bc, _ := bcm.LoadOrNewBlockchain(state.db, testGenesisDoc, logger)
...
func makeExecutor(state *State) *executor {
	return newExecutor("makeExecutorCache", true, state, testChainID,
		newBlockchain(testGenesisDoc, state), event.NewEmitter(logger), logger)
}
...
func TestSelfDestruct(t *testing.T) {
	...
	exe := NewBatchCommitter(state, testChainID, newBlockchain(testGenesisDoc, state), event.NewNoOpPublisher(), logger)
...
func execTxWithState(state *State, tx txs.Tx) error {
	return execTxWithStateAndBlockchain(state, newBlockchain(testGenesisDoc, state), tx)
}
