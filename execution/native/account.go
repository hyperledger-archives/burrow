...
func CreateAccount(st acmstate.ReaderWriter, address crypto.Address) error {
	...
	if acc != nil {
		...
		return errors.Errorf(errors.Code.DuplicateAddress,
...
func initCode(st acmstate.ReaderWriter, address crypto.Address, parent *crypto.Address, code []byte) error {
	...
	if parent != nil {
		...
		if ancestor == nil {
			return errors.Errorf(errors.Code.NonExistentAccount,
	...
	if metamap != nil && len(metamap) > 0 {
		...
		if !found {
			return errors.Errorf(errors.Code.InvalidContractCode,
...
func Transfer(st acmstate.ReaderWriter, from, to crypto.Address, amount uint64) error {
	...
	if acc.Balance < amount {
		return errors.Code.InsufficientBalance
	}
...
func UpdateAccount(st acmstate.ReaderWriter, address crypto.Address, updater func(*acm.Account) error) error {
	acc, err := mustAccount(st, address)
	if err != nil {
		return err
	}
...
func mustAccount(st acmstate.Reader, address crypto.Address) (*acm.Account, error) {
	...
	if acc == nil {
		return nil, errors.Errorf(errors.Code.NonExistentAccount,
