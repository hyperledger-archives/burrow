...
func EnsureKeyFormatStore(ks interface{}) ([][]byte, error) {
	...
	prefixes := make([][]byte, rt.NumField())
	keyFormats := make(map[string]MustKeyFormat)
	for i := 0; i < rt.NumField(); i++ {
		fv := rv.Field(i)
		if fv.Kind() == reflect.Ptr {
			if fv.IsNil() {
				return nil, fmt.Errorf("key format field '%s' is nil", rt.Field(i).Name)
			}
			fv = fv.Elem()
		}
		ft := fv.Type()
		if ft == expectedKeyFormatType {
			...
			prefixes = append(prefixes, kf.Prefix())
			prefix := kf.Prefix().String()
			if kfDuplicate, ok := keyFormats[prefix]; ok {
				return nil, fmt.Errorf("duplicate prefix %q between key format %v and %v",
...
func (kf *KeyFormat) KeyBytes(segments ...[]byte) ([]byte, error) {
	...
	for i, l := range kf.layout {
		...
		default:
			if len(s) != l {
				return nil, fmt.Errorf("the key segment '%X' (length %d) provided to KeyFormat.KeyBytes() "+
					"does not the have required %d bytes required by layout for segment %d", s, len(s), l, i)
			}
			n += l
