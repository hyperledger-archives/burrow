...
describe('ABI helpers', () => {
  ...
  it('Derive Typescript contract type from ABI', () => {
    ...
    type FunctionName = Func['name'];

    type FuncByName<T, Name extends FunctionName> = T extends { name: Name; type: 'function' } ? T : never;

    type Picker<name extends FunctionName> = {
      [i in keyof abi]: FuncByName<abi[i], name>;
    };
    ...
    type Type<T> = T extends 'uint256' ? number : T extends 'bool' ? boolean : T extends 'address' ? Address : never;
    ...
    type Args<Name extends FunctionName> = TypesOf<FunctionInputs<Name>>;
    ...
    type Contract = {
      ...
      // [name in FunctionName]: (...args: Args<name>) => TypesOf<FunctionOutputs<name>>
