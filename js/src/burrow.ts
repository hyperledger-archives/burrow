...
export class Burrow {
  ...
  constructor(URL: string, account: string) {
    ...
    this.ec = Burrow.newClient('rpcevents', 'ExecutionEvents', URL);
    this.tc = Burrow.newClient('rpctransact', 'Transact', URL);
    this.qc = Burrow.newClient('rpcquery', 'Query', URL);
  ...
  private static newClient<T>(pkg: string, service: string, URL: string): T {
