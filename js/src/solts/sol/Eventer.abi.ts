...
import { Address, CancelStreamSignal, ContractCodec, Event, listenerFor } from '../../index';
interface Provider {
  deploy(
...
export namespace Eventer {
  ...
  export function deploy(client: Provider): Promise<string> {
    ...
    return client.deploy(data, [{ abi: Eventer.abi, codeHash: Buffer.from(Eventer.deployedBytecode, 'hex') }]);
  }
